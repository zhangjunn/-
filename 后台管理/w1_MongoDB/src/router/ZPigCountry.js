...
Router.post('/reg',async (req,res)=>{
    ...
    let data
    try{
        insert('ZPigCountry',{username,password,age,gender,city,telephone});//{username,password,age,gender}
        res.send(formatData())
    }catch(err){
...
Router.get('/check',async (req,res)=>{
    ...
    let data
    try{
        data = await find('ZPigCountry',{username});//{username,password,age,gender}
        data = data[0];
        if(data){
            res.send(formatData({code:0}))
        }else{
...
Router.post('/login',async (req,res)=>{
    ...
    try{      //这里user数据库名字  用户名和密码 
        data = await find('ZPigCountry',{username,password});//{username,password,age,gender}
...
Router.get('/',async (req,res)=>{
    ...
    // let data = await find('ZPigCountry',{},{skip:1,limit:2,sort:"age",asc:true});  //asc:true 升序  （false）降序
    let data = await find('ZPigCountry',{},{skip,limit,sort}); 
    res.send(formatData({data}))
})
Router.get('/:id',async (req,res)=>{ 
    let {id} = req.params;
    let data = await find('ZPigCountry',{_id:id});
    res.send(formatData({data}))
})
...
//     let data = await update('ZPigCountry', {
//         id:id
